...
class GlocalTextPathCMT(BertPreTrainedModel):
    ...
    def forward(
        ...
        for step in range(max_step):
            ...
            knowledge_ft = torch.cat([knowledge_fts[b][min(step,knowledge_fts[b].shape[0]-1):min(step,knowledge_fts[b].shape[0]-1)+1] for b in range(batch_size)],dim=0)
            crop_ft = torch.cat([crop_fts[b][min(step,crop_fts[b].shape[0]-1):min(step,crop_fts[b].shape[0]-1)+1] for b in range(batch_size)],dim=0)
            ...
            crop_knowledge_ft = self.cross_vision_knowledge(knowledge_ft.view(batch_size*36,CROP_SIZE*CROP_SIZE,768),None,crop_ft.view(batch_size*36,CROP_SIZE,768),None).view(batch_size,36,CROP_SIZE,768)

            if step != 0:
                history_knowledge_ft = self.cross_history_knowledge(knowledge_ft.view(batch_size*36,CROP_SIZE*CROP_SIZE,768),None,history_fts.view(batch_size*36,-1,768),None).view(batch_size,36,-1,768)
