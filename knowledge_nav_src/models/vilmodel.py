...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    ...
    def forward_panorama_per_step(
        ...
        # Interaction
        crop_knowledge_fts = self.cross_vision_knowledge(knowledge_fts.view(batch_size*36,5*CROP_SIZE,768),None,crop_fts.view(batch_size*36,CROP_SIZE,768),None).view(batch_size,36,CROP_SIZE,768)

        if gmap_img_embeds != None:
            history_knowledge_fts = self.cross_history_knowledge(knowledge_fts.view(batch_size*36,5*CROP_SIZE,768),None,history_fts.view(batch_size*36,-1,768),None).view(batch_size,36,-1,768)
        ...
        if gmap_img_embeds != None:
            ...
            fusion_fts = torch.cat([crop_knowledge_fts,self.no_history_embedding.unsqueeze(0).repeat(batch_size,36,1)],dim=-1)
